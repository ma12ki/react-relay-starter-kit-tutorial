...
/*
 * Next, let's define a node interface and type. We need only provide a way for Relay 
 * to map from an object to the GraphQL type associated with that object, and from a global ID to the object it points to:
 */
...
/*
 * With the queries out of the way, let's start in on our only mutation: the one that spends a turn by checking a spot for treasure. 
 * Here, we define the input to the mutation (the id of a spot to check for treasure) and a list of all 
 * of the possible fields that the client might want updates about after the mutation has taken place. 
